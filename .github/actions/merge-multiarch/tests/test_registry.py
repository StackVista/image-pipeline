...
@unittest.skipUnless(os.environ.get('PUBLICATION_TEST_REGISTRY'), 'requires an isolated local registry')
    ...
    def test_resume_keeps_manifest_and_successful_signature(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            def command(*args, **kwargs):
                ...
                if args[:2] == ('cosign', 'sign') and args[-1] == final_reference[0] and '--new-bundle-format=true' in args and not interrupted:
                    interrupted = True
            ...
            with patch.object(publish, 'command', command):
                for arch in ('amd64', 'arm64'):
                    ...
                    (receipts / f'{arch}.json').write_text(json.dumps(dict(
                        image=image, tag='v1', arch=arch, digest=digest, repository=os.environ['GITHUB_REPOSITORY'],
                ...
                before = real_command('docker', 'buildx', 'imagetools', 'inspect', image + ':v1', '--raw').stdout
                publish.publish(image, 'v1', ['amd64', 'arm64'], receipts)
                publish.publish(image, 'v1', ['amd64', 'arm64'], receipts)
                after = real_command('docker', 'buildx', 'imagetools', 'inspect', image + ':v1', '--raw').stdout
                self.assertEqual(before, after)
                # A mutated architecture tag cannot change the recorded publication.
                real_command('docker', 'buildx', 'imagetools', 'create', '-t', image + ':v1-amd64', image + ':v1-arm64')
                publish.publish(image, 'v1', ['amd64', 'arm64'], receipts)
                self.assertEqual(before, real_command('docker', 'buildx', 'imagetools', 'inspect', image + ':v1', '--raw').stdout)
