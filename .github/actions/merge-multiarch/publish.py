...
def inspect(reference, missing_ok=False):
    result = command(
        "docker", "buildx", "imagetools", "inspect", reference, "--raw", allow_failure=True
    )
    ...
    if (
        ...
        and (
            ...
            or re.search(r": not found\s*$", error)
        )
    ):
        return None
...
def verify_signature(reference, bundle, identity):
    result = command(
        "cosign",
        "verify",
        f"--new-bundle-format={str(bundle).lower()}",
    ...
    if result.returncode == 0 and bundle:
        ...
        if not any(
            item.get("critical", {}).get("type") == "https://sigstore.dev/cosign/sign/v1"
...
def ensure_signatures(reference, identity):
    for bundle in (False, True):
        ...
        command(
            ...
            "--yes",
            f"--new-bundle-format={str(bundle).lower()}",
            f"--use-signing-config={str(bundle).lower()}",
            reference,
        )
        result = verify_signature(reference, bundle, identity)
...
def validate_platforms(manifest, arches):
    ...
    platforms = [
        (child.get("platform", {}).get("os", ""), child.get("platform", {}).get("architecture", ""))
        for child in children
        if child.get("annotations", {}).get("vnd.docker.reference.type") != "attestation-manifest"
    ]
...
def publish(image, tag, arches, receipt_dir):
    ...
    targets = (
        recorded_targets(receipt_dir, image, tag, arches)
        if receipt_dir
        else [f"{release}-{arch}" for arch in arches]
    )
    identity = (
        "^"
        + re.escape(os.environ["GITHUB_SERVER_URL"] + "/" + os.environ["GITHUB_REPOSITORY"])
        + r"/\.github/workflows/[^@]+@"
    ...
    expected_raw = command(
        "docker", "buildx", "imagetools", "create", "--dry-run", "-t", release, *targets
    ).stdout
    ...
    digest = command(
        "docker", "buildx", "imagetools", "inspect", release, "--format", "{{.Manifest.Digest}}"
    ).stdout.strip()
    if (
        not re.fullmatch(r"sha256:[0-9a-f]{64}", digest)
        or json.loads(inspect(f"{image}@{digest}")) != expected
    ):
    ...
    if summary := os.environ.get("GITHUB_STEP_SUMMARY"):
        with open(summary, "a") as stream:
            stream.write(
                f"- Published and verified `{release}` (`{digest}`; {', '.join(arches)})\n"
            )
...
if __name__ == "__main__":
    try:
        publish(
            os.environ["INPUTS_IMAGE"],
            os.environ["INPUTS_TAG"],
